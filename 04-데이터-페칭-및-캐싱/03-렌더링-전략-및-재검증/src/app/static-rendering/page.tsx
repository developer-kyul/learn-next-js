...
export const dynamic = 'force-static' // 현재 페이지 강제로 정적 렌더링
        ...
        <div className="md:flex-1">
          ...
          <div className="mt-2 mb-3 flex flex-col gap-1 text-sm text-slate-700">
            ...
            <p className="flex items-center gap-1">
              <LucideInfo className="size-4" />이 페이지는 빌드 시점에 한 번만
              생성되며, 모든 사용자에게 동일한 캐시 데이터를 제공합니다.
            </p>
            <p className="ml-5 text-xs text-slate-500">
              <code className="rounded-sm bg-slate-100 p-0.5 px-1">
                bun run build && bun start
              </code>{' '}
              환경에서 서버 로그를 통해 추가적인 데이터 요청이 발생하지 않음을
              확인할 수 있습니다.
            </p>
          </div>
        </div>
