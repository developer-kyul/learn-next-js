...
export default async function SetupCheckPage() {
  ...
  try {
    ...
    errorMessage = isErrorObject(error)
      ? error.message
      : 'Supabase 연동에 실패했습니다.'
  }
  ...
  return (
    <section
      className={cn('w-80/100 max-w-3xl px-6 py-12 antialiased', 'mx-auto')}
    >
      <header className="mb-10">
        ...
        <div className="flex items-center gap-4">
          <div
            className={cn(
              'inline-flex size-14 items-center justify-center rounded-2xl border border-slate-100 bg-slate-50 shadow-sm',
              isConnected
      ...
      <div className="relative z-10">
        ...
        <p className="mb-6 text-sm leading-relaxed font-medium text-slate-500">
          {isConnected ? (
            ...
            <>
              아래 환경 변수가{' '}
              <code className="rounded bg-rose-50 px-1.5 py-0.5 font-mono text-xs text-rose-700">
                .env
              </code>
              또는{' '}
              <code className="rounded bg-rose-50 px-1.5 py-0.5 font-mono text-xs text-rose-700">
        ...
        <ul className="mb-8 space-y-3">
          <li className="flex items-center justify-between border-b border-slate-50 pb-2.5 text-sm font-semibold">
            ...
            >
              {supabaseConfig?.url ? (
                <LucideCheckCircle2 className="mr-1.5 size-4" />
              ) : (
                <LucideXCircle className="mr-1.5 size-4" />
              )}
        ...
        {errorMessage && (
          <div className="mb-6 rounded-2xl border border-rose-100 bg-rose-50 p-4 font-mono text-xs text-rose-600">
            ...
            <p className="pl-0.5 leading-relaxed break-all">{errorMessage}</p>
          </div>
        )}
